...
说明：表格标题横向，内容纵向，包含「实际结果」字段，按模块拆分，格式规范可直接编辑。

1. 登录模块（单接口测试·tests/api/）

| 用例编号 | 用例名称 | 前置条件 | 步骤 | 预期结果 | 实际结果 | 断言点 | 测试命令 |
|----------|----------|----------|------|----------|----------|--------|----------|
